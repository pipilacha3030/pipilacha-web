...
- **Reveal pattern**: any element with class `.reveal` starts at `opacity:0; translateY(32px)`, but that hide-state is **gated on the `.js` class** (`.js .reveal{…}`) so content stays visible without JS or if the bundle fails to download (el inline script de Base.astro escucha errores de carga de `<script src>` y retira `.js`; **si editas ese inline, recalcula su hash sha256 en la CSP de `public/.htaccess`**). A generic ScrollTrigger in `main.js` animates each `.reveal` in. Put `[data-reveal-stagger]` on a container to reveal its `.reveal` children in cascade instead of one-by-one. Add `.reveal` to opt in. Hero elements use `data-delay` (time-based, not scroll-based) and are handled separately.
