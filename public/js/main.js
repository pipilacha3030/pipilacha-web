...
if (window.gsap) {
  ...
  if (!reduceMotion) {
    ...
    /* ── FOTOS EN SCROLL: collage 3D · tres planos + flores + escala de profundidad ── */
    const imgFlow = document.querySelector('.img-flow');
    if (imgFlow) {
      const far  = imgFlow.querySelector('.track--far');
      const mid  = imgFlow.querySelector('.track--mid');
      const near = imgFlow.querySelector('.track--near');
      const isVisible = el => el && getComputedStyle(el).display !== 'none';

      /* ── 1. Desplazamiento horizontal por capas (velocidades distintas = parallax) ── */
      const farTween = gsap.fromTo(far,
        { x: () => window.innerWidth * 1.0 },
        { x: () => -(far.offsetWidth - window.innerWidth * 0.18), ease: 'none',
          scrollTrigger: { trigger: imgFlow, start: 'top top', end: 'bottom bottom', scrub: 2, invalidateOnRefresh: true } }
      );

      const midTween = gsap.fromTo(mid,
        { x: () => window.innerWidth * 1.1 },
        { x: () => -(mid.offsetWidth - window.innerWidth * 0.20), ease: 'none',
          scrollTrigger: { trigger: imgFlow, start: 'top top', end: 'bottom bottom', scrub: 2, invalidateOnRefresh: true } }
      );

      let nearTween = null;
      if (isVisible(near)) {
        nearTween = gsap.fromTo(near,
          { x: () => window.innerWidth * 1.25 },
          { x: () => -(near.offsetWidth - window.innerWidth * 0.14), ease: 'none',
            scrollTrigger: { trigger: imgFlow, start: 'top top', end: 'bottom bottom', scrub: 2, invalidateOnRefresh: true } }
        );
      }

      /* ── 2. Rotación inicial de flores (GSAP toma el control del transform) ── */
      imgFlow.querySelectorAll('.img-flow__flower[data-rotate]').forEach(fl => {
        gsap.set(fl, { rotate: parseFloat(fl.dataset.rotate) });
      });

      /* ── 3. Escala de profundidad: pequeño→grande al centro→pequeño ── */
      const addScaleFlow = (track, trackTween, minScale) => {
        if (!track || !trackTween) return;
        gsap.utils.toArray(track.querySelectorAll('.img-flow__item, .img-flow__flower')).forEach(el => {
          gsap.set(el, { scale: minScale, transformOrigin: 'center center' });
          ...
          tl.to(el, { scale: 1,        ease: 'sine.out', duration: 0.5 });
          tl.to(el, { scale: minScale, ease: 'sine.in',  duration: 0.5 });
        });
      };

      addScaleFlow(far,  farTween,  0.45);
      addScaleFlow(mid,  midTween,  0.48);
      if (nearTween) addScaleFlow(near, nearTween, 0.40);

      /* ── 4. Rotación orgánica de fotos del plano cercano al cruzar el viewport ── */
      if (nearTween) {
        const nearRots = [-1.5, 2.5, -2, 1.5];
        gsap.utils.toArray('.track--near .img-flow__item').forEach((item, i) => {
          const base = nearRots[i] ?? 0, swing = 5, dir = i % 2 === 0 ? 1 : -1;
          gsap.fromTo(item,
            { rotate: base + dir * swing },
            { rotate: base - dir * swing, ease: 'none',
              scrollTrigger: { containerAnimation: nearTween, trigger: item, start: 'left right', end: 'right left', scrub: true }
            }
          );
        });
      }

      /* ── 5. Mouse move: parallax vertical por plano — da volumen en 3D ── */
      if (!reduceMotion && window.matchMedia('(hover:hover) and (pointer:fine)').matches) {
        const qFar  = gsap.quickTo(far, 'y', { duration: 2.4, ease: 'power2.out' });
        const qMid  = gsap.quickTo(mid, 'y', { duration: 1.6, ease: 'power2.out' });
        const qNear = isVisible(near) ? gsap.quickTo(near, 'y', { duration: 1.0, ease: 'power2.out' }) : null;
        imgFlow.addEventListener('mousemove', e => {
          const cy = e.clientY / window.innerHeight - 0.5;
          qFar(cy * 20); qMid(cy * -34); if (qNear) qNear(cy * 52);
        });
        imgFlow.addEventListener('mouseleave', () => {
          qFar(0); qMid(0); if (qNear) qNear(0);
        });
      }
    }
